...
// Returns all extension entries currently rendered in the panel
function getExtensionEntries() {
    return Array.from(document.querySelectorAll('#extensions_list .extension_block, #extensions_list .inline-drawer'));
}

// Read stable IDs from rendered extension blocks
// ST uses the folder name as data-name or as text
function readExtensionIds() {
    const entries = getExtensionEntries();
    return entries.map(el => {
        const nameEl = el.querySelector('.extension_name, .inline-drawer-toggle b, b');
        const name = nameEl ? nameEl.textContent.trim() : el.dataset.name || '';
        return name;
    });
}
...
function applyOrder() {
    ...
    if (!container) {
        toastr.error('Extensions 패널을 찾을 수 없습니다. Extensions 탭을 열고 다시 시도하세요.');
        return;
    }

    // Build a map: name → element
    const allBlocks = Array.from(container.querySelectorAll('.extension_block, .inline-drawer'));
    const nameToEl = {};
...
function injectManagerButton() {
    ...
    // Look for the extensions panel header area
    const extHeader = document.querySelector('#extensionsMenu .inline-drawer-toggle, #extensionsMenuButton, #extensions-menu-button');
    // Fallback: inject a floating button near the Extensions icon in the topbar
    const topbar = document.getElementById('extensionsMenuButton') || document.querySelector('[id*="extension"][id*="menu"]');

    // We'll add the button to the extensions panel title bar or top of the panel
    const extPanel = document.querySelector('#extensionMenu, #extensionsMenu, .extensions-block');
    if (!extPanel) return;
    ...
    btn.innerHTML = '<i class="fa-solid fa-sliders"></i> 관리자';
    btn.addEventListener('click', openExtensionManager);

    // Insert at the top of the extensions panel
    extPanel.insertAdjacentElement('afterbegin', btn);
}
...
(function init() {
    ...
    if (document.querySelector('#extensionMenu, #extensionsMenu, .extensions-block')) {
        injectManagerButton();
    }
